...
右上角「批量执行」打开复选框列表，支持逐台勾选、按名称/IP/分组筛选后全选、取消全部。默认不选机器；筛选不会自动清除已选项。填写单次命令或使用 pgrep / pkill 模板，再点「查看执行范围」，核对完整命令、机器地址与 SSH 用户，最后点「在 N 台机器上执行一次」。
...
- 复用现有共享凭据，只允许已启用机器；每台独立 SSH 连接，最多 4 台并发。不会写入定时 watch 配置，不会打断监控连接。
- `pkill` 按当前账号权限终止匹配的进程。可先执行 pgrep 核对；模板中的匹配表达式由用户填写，页面不会自动执行模板。
- 每台显示等待、执行中、成功、失败、结果未知及退出码、stdout/stderr。命令超时 12 秒；超时/断线/缺失退出状态不自动重试。同一执行编号重复提交只返回原结果，不会再执行；同一编号换命令或目标会被拒绝。
- 使用已信任的主机密钥；陌生或变更密钥须先在监控流程处理。提交时再次检查启用状态、IP、端口和用户名，目标配置变化时拒绝执行。重复 SSH 地址与用户名不能在同一批中执行两次。
- 同时只允许一批；关闭弹窗后后台继续，刷新页面可从「最近执行」找回本次程序运行的结果（最多 5 批，单条输出流最多 16 KiB）。程序重启不恢复或重放批量命令。关闭 SSH 连接无法保证已脱离会话的远程子进程退出。
- 一次性执行不接受 watch 前缀；需要定时采集时使用原来的「自定义」监控栏。演示模式仅供选择范围和查看命令，不会提交 SSH 执行。
