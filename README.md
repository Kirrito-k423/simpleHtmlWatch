...
- 指令栏仅在自定义视图显示，占用一行；卡片放大、全屏及正文定位同样适用。
- 指令和运行状态保存在本机加密配置中，刷新页面、重启程序后恢复。编辑输入框不会立即执行，按「保存并运行」后才替换当前指令。
- 切换回 NPU 等视图或暂停页面显示，仍继续采集；点「停止」停止后续自定义采集并保留指令文本，内置监控继续。停止会关闭正在使用的 SSH 通道，无法保证远端自行脱离会话的子进程退出。
- 指令随配置导出、导入；导入后默认停止，查看内容后再从自定义栏启动。
- 自定义命令应为可重复执行的短查询。使用所配置的 SSH 账号权限执行，程序不会判定命令是否只读；每次最多运行 12 秒，保留最多 256 KiB 输出。自定义命令断线后本轮不额外重放，等下一轮再执行。
