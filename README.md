...
- **失败可见**：离线、密码错误、主机密钥变化、命令缺失分别显示；单台出错不阻塞其余机器。命令执行时连接中断会在本轮重连并重试一次，持续失败再等待下轮；不无限积累任务。
...
每次命令执行（含必要的连接探测）超时 12 秒，连接超时 8 秒，SSH 握手超时 10 秒；发生传输错误时每轮最多重连重试一次，因此该轮采集耗时可能增加；单命令输出最多 256 KiB。界面展示原始输出，暂不解析型号专属指标、不保存历史曲线，也不提供任意命令执行、杀进程或交互终端。关闭 SSH 通道通常会终止这些短命令，但无法保证任意远端子进程都收到终止信号。
...
- 连接已断开或复用的会话失效时，本轮最多新建一次 SSH 连接并重试当前只读监控命令。主机密钥校验仍正常执行。
