...
5. 首次连接点「核对指纹」，与管理员确认 SSH 主机指纹后开始采集。
...
- **自动刷新**：默认 5 秒，允许 3–3600 秒；保持 SSH 连接，最多 8 台同时采集。一次采集完成后等待配置间隔，因此慢命令会延长实际刷新周期。
...
- `vault.key`：本机随机加密密钥。
- `known_hosts.json`：用户确认过的 SSH 主机指纹。
...
SSH 在发送密码前校验主机密钥。第一次连接需要用户核对指纹；已有机器密钥变化时阻止认证并要求重新核对。不要仅因提示而盲目信任新指纹。当前只支持 SSH 密码认证（含密码式 keyboard-interactive），不支持密钥、跳板机、MFA 问答或 SSH config。
