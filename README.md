...
每次命令执行（含必要的连接探测）超时 12 秒，连接超时 8 秒，SSH 握手超时 10 秒；发生传输错误时每轮最多重连重试一次，因此该轮采集耗时可能增加；单命令输出最多 256 KiB。界面展示原始输出，暂不解析型号专属指标、不保存历史曲线，不提供交互终端。关闭 SSH 通道通常会终止这些短命令，但无法保证任意远端子进程都收到终止信号。
...
点击「保存并运行」或按 Enter，即可在**所有已启用机器**上定时执行并统一展示结果（搜索、分组和分页只影响显示范围）。默认每轮完成后等待 4 秒，可在管理机器中调整间隔。支持 Bash 管道、引号和筛选语法，远端沿用 `bash -o pipefail -lc`，不需要加 `watch` 前缀。若不希望匹配 grep 自身，可使用 `ps -ef | grep '[t]ilexr'`；grep 无匹配时会返回退出码 1，页面会如实显示。
